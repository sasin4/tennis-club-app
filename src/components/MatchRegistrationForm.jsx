...
import { insertMatchResult } from '../api/tennisApi';

export default function MatchRegistrationForm({ onBack, onSubmitSuccess }) {
  const [matchDate, setMatchDate] = useState('2026-05-28'); // 현재 날짜 기준 기본값
  ...
  const handleSubmit = async (e) => {
    ...
    // 1. 기본 라인업 유효성 검사
    if (!opponent1 || (matchType === 'Doubles' && (!partner || !opponent2))) {
    ...
    const payload = {
      ...
      partner_id: matchType === 'Doubles' ? partner : null, // 실제 서비스에선 ID 데이터 유입
      opponent1_id: opponent1,
      opponent2_id: matchType === 'Doubles' ? opponent2 : null,
        ...
        <section className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="date" value={matchDate} onChange={(e) => setMatchDate(e.target.value)} disabled={isSubmitting}
                     className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-[#002B5C] outline-none disabled:bg-gray-100" required />
            </div>
            <div>
        ...
        <section className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-2 gap-4 relative">
            ...
            <div className="space-y-3 bg-blue-50 p-3 rounded-xl border border-blue-100">
              ...
              {matchType === 'Doubles' && (
                <input type="text" value={partner} onChange={(e) => setPartner(e.target.value)} placeholder="파트너 이름" disabled={isSubmitting}
            ...
            <div className="space-y-3 bg-gray-50 p-3 rounded-xl border border-gray-200">
              ...
              <input type="text" value={opponent1} onChange={(e) => setOpponent1(e.target.value)} placeholder="상대편 1" disabled={isSubmitting}
                     className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm text-center focus:ring-2 focus:ring-[#002B5C] outline-none disabled:bg-gray-100" required />
              {matchType === 'Doubles' && (
                <input type="text" value={opponent2} onChange={(e) => setOpponent2(e.target.value)} placeholder="상대편 2" disabled={isSubmitting}
                       className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm text-center focus:ring-2 focus:ring-[#002B5C] outline-none disabled:bg-gray-100" required />
              )}
            </div>
          </div>
          ...
          <div className="space-y-3">
            {sets.map((set, index) => (
              <div key={set.id} className="flex items-center gap-2 bg-gray-50 p-2 rounded-xl border border-gray-200 relative">
                ...
                {sets.length > 2 && index === sets.length - 1 && !isSubmitting && (
